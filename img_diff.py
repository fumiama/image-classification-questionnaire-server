...
def get_dhash_b14_io(dataio: BytesIO):
    b14_dhash = "0" * 16
    with Image.open(dataio) as img:
        b14_dhash = base14.get_base14(a2b_hex(str(dhash(img))))[:-1]
    return b14_dhash

def decode_dhash(img_name_no_ext_b14: str):
    return base14.from_base14((img_name_no_ext_b14 + last_char).encode("utf-16-be"))

@jit
def hamm_img(byte_res1: bytes, byte_res2: bytes, length: int):
    """
